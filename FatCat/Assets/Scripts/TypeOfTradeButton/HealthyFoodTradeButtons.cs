...
public class HealthyFoodTradeButtons : MonoBehaviour
{
    ...
    public void Trade(string priceInFoodAndPriceInCoins)
    {
        ...
        ProcessingPlayerData.SetDelegateShowMessage(new ProcessingPlayerData.ShowMessage(Show));
        ProcessingPlayerData.Trade(Convert.ToInt32(splitPrices[0]), "healthyfood", out int? price, Convert.ToInt32(splitPrices[1]));
        if (price == null)
        {
            Show("");
        }
